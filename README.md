...
Provision a PostgreSQL database and create the `authenticated_user` role before running migrations. For local migrations, copy `.env.example` to `.env` and set `DATABASE_URL` to the database connection string. For `wrangler dev`, also set `CLOUDFLARE_HYPERDRIVE_LOCAL_CONNECTION_STRING_DATABASE_HYPERDRIVE` to that same connection string. Local development should use an explicit environment, such as `npm run dev -- --env staging`, so the corresponding KV bindings are loaded.
