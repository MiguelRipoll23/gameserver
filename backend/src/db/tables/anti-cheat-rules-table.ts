 ...
 * the `4030BF2F` key of the game configuration. Violations of these rules are
 * always recorded as automatic reports — the server never bans from a client
 * report. Management-only data, so no RLS policies are needed (mirrors
 * `blocked_words`).
 */
export const antiCheatRulesTable = pgTable("anti_cheat_rules", {
